...
if __name__ == '__main__' :
	...
	def stats():
		...
		print 'Total number of tweets (1.788.070)'
		print dbnp.tweets.find({'text' : {'$ne':None}}).count()
		...
		print 'Number of tweets with geolocation/coordinates (31.999)'
		print dbnp.tweets.find({'geo':{'$ne':None}}).count()
		...
		print dbnp.tweets.aggregate([{"$group": {"_id": "$lang", "count": {"$sum": 1}}},{"$sort": SON([("count", -1), ("_id", -1)])}])['result'][:100]
		print 'Country Stats'
		print dbnp.tweets.aggregate([{"$group": {"_id": "$place.country", "count": {"$sum": 1}}},{"$sort": SON([("count", -1), ("_id", -1)])}])['result'][:100]
		print 'City Stats'
		print dbnp.tweets.aggregate([{"$group": {"_id": "$place.name", "count": {"$sum": 1}}},{"$sort": SON([("count", -1), ("_id", -1)])}])['result'][:100]
		print 'Number of Unique users (766836)'
		users = []
		for user in dbnp.tweets.find({'text':{'$ne':None}},{'user.screen_name':1}):
